      ...
      <div class="align-middle inline-block min-w-full shadow overflow-hidden rounded border-b border-gray-200">
        <table class="table-fixed min-w-full">
          <thead class="bg-color-image text-color-button">
            <tr>
              <th class="w-3/12 px-6 py-3 border-b border-color-form text-left text-xs leading-4 font-medium uppercase tracking-wider">
                {{ $t('name') }}
              </th>
              <th class="w-2/12 px-6 py-3 border-b border-color-form text-left text-xs leading-4 font-medium uppercase tracking-wider">
                {{ $t('category') }}
              </th>
              <th class="w-3/12 px-6 py-3 border-b border-color-form text-left text-xs leading-4 font-medium uppercase tracking-wider">
                {{ $t('status') }}
              ...
              <th class="w-2/12 px-6 py-3 border-b border-color-form"></th>
            </tr>
          </thead>
          <tbody class="bg-color-form">
            <tr v-for="(u, index) in users" :key="index" :class="{'shadow': u.is_active}">
              ...
              <td v-if="u.is_superuser" class="px-6 py-4 whitespace-no-wrap border-b border-color-formtext-sm leading-5 text-color-body">
                Superuser
              </td>
              <td v-else class="px-6 py-4 whitespace-no-wrap border-b border-color-formtext-sm leading-5 text-color-body">
                User
              </td>

              <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-color-formtext-sm leading-5 font-medium">
