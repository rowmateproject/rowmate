...
<div v-click-outside="toggleSearch" @keydown.esc="toggleSearch" class="relative">
  <div class="flex flex-wrap items-stretch relative w-full mt-2 lg:mt-3">
    <input v-model="searchTerm" @input="lookupEvent" type="text" :class="{'border border-color-form' : hasBorder}" class="flex-shrink flex-grow flex-auto leading-normal flex-1 border-r-0 rounded md:rounded-r-none md:rounded-l focus:outline-none p-2">
