...
async def patch_event(hex: str,
    ...
    if len(res) == 0:
        raise HTTPException(status_code=404, detail='Calender was not found')
    ...
    for event_time_index, event_time in enumerate(event_times):
        ...
        if existing_events > 0:
            ...
            update_res = await db['events'].update_many(query, document)
            ...
            insert_res = await db['events'].insert_one(event_doc)
    ...
    if len(delete_uuids) > 0:
        for uuid in delete_uuids:
            ...
            delete_res = await db['events'].delete_one(query)
    ...
    if len(inserted) > 0:
        ...
        await db['calendars'].update_one(query, document)
